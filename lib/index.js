      ...
      return getLangResource(langDir, opt).then((langResource) => {
        ...
        if (file._lang_ && file.runId === runId) {
          ...
          _langs_.forEach((lang) => {
            ...
            insidePath = ""
            if (typeof(opt.insidePath) === 'string') {
              insidePath = opt.insidePath
            }
            if (opt.createLangDirs) {
              newFilePath = path.join(resolveFileBase(file.base), lang, insidePath, getRelativeFilePath(newFilePath, file.base));
              if (opt.filenameI18n) {
